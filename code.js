function autoFillIPSTemplateGoogleDoc(e) {
  ...
  let goal1 = "Save for retirement" //value is 1 point
  let goal2 = "Fund my children's education" //value is 2 points
  let goal3 = "Generate income" //value is 3 points
...
//added code based on answers from multiple choice. The risk level is determined by the total sum of the answers. 
let totalsum = 0
if (goal1 === true) {
  totalsum++
} else if (goal2 === true) {
...
if (totalsum <= 45 && totalsum >= 27) {
    risk = low
} else if (totalsum > 45 && totalsum <= 63) {
    risk = medium 
} else if ( totalsum > 63 && totalsum <= 81) {
    risk = high
};